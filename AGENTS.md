...
| `internal/harness` | Agent-config **adapters**: `claude.go`, `opencode.go`, `codex.go` (`[mcp_servers.*]` TOML), `crush.go`, `forge.go`, `hermes.go`, `copilot.go`, `qwen.go`, `gemini.go`, `kilo.go` (JSONC), `kimi.go` (TOML), `zcode.go` (nested `mcp.servers` JSON). `harness.go` holds the `Adapter` interface (`List` + `Apply`), `DefaultPath`, format-neutral `MCPServer`, diff planning, and backup; `jsonutil.go` preserves unknown keys and strips JSONC comments. |
